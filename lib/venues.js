...
// Get list of supported cities with state codes for display
export function getSupportedCities() {
  return Object.entries(VENUE_DATA).map(([key, data]) => ({
    city: key.split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' '),
    stateCode: data.stateCode,
    venueCount: data.venues.length,
    label: `${key.split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}, ${data.stateCode}`,
  }));
}
