...
// --- Event building ---
function buildCalendarEvent(show, settings = {}) {
  ...
  // Reminders
  if (settings.reminderEnabled && settings.reminderMethods?.length > 0 && settings.reminderMinutes?.length > 0) {
    event.reminders = {
      useDefault: false,
      overrides: settings.reminderMethods.flatMap(method =>
        settings.reminderMinutes.map(minutes => ({ method, minutes }))
      ),
    };
...
export async function syncShowToCalendar(show, settings, persistSettings) {
  ...
  const event = buildCalendarEvent(show, settings);
